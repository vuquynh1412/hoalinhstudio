...
export function HomePage({ locale }: HomePageProps) {
  ...
  const hiddenHeaderNavItems = headerNavigationItems.slice(visibleHeaderNavCount);
  ...
  useEffect(() => {
    ...
    const frameId = window.requestAnimationFrame(() => {
      if (nav.scrollWidth > nav.clientWidth + 1) {
        setVisibleHeaderNavCount((currentCount) => Math.max(currentCount - 1, 0));
      }
    });
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={cn(
            "mx-auto flex h-16 w-full max-w-[1440px] items-center justify-between gap-3 px-4 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] sm:h-[72px] sm:px-6 lg:gap-5 lg:px-10",
            isHeaderVisible
          ...
          <div className="hidden min-w-0 flex-1 lg:flex">
            ...
            >
              {visibleHeaderNavItems.map((item) => (
                <a
                  key={item.href}
                  className="focus-ring inline-flex shrink-0 items-center justify-center whitespace-nowrap rounded-full px-3 py-2 text-[15px] font-[500] text-black/78 transition-colors duration-200 hover:text-black"
                  href={item.href}
                >
                  {item.label}
                </a>
      ...
      <section className="hero-screen relative isolate flex overflow-hidden bg-[#f8f7f3] pt-16 text-[#171717] sm:pt-[72px]">
        <video
          aria-hidden="true"
          autoPlay
        ...
        <div className="hero-screen relative z-30 flex w-full flex-col px-4 pb-4 sm:px-5 sm:pb-5">
