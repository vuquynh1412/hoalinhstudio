...
const aboutRevealWords = aboutRevealText.split(" ");

const normalizeAboutWord = (word: string) =>
  word
    .normalize("NFC")
    .toLocaleLowerCase("vi-VN")
    .replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu, "");

const aboutHighlightWordIndexes = new Set(
  aboutHighlightPhrases.flatMap((phrase) => {
    const phraseWords = phrase
      .split(" ")
      .map(normalizeAboutWord)
      .filter(Boolean);
    ...
    for (
      let startIndex = 0;
      startIndex <= aboutRevealWords.length - phraseWords.length;
      startIndex += 1
    ) {
      const isMatch = phraseWords.every((phraseWord, offset) => {
        return (
          normalizeAboutWord(aboutRevealWords[startIndex + offset]) ===
          phraseWord
        );
      });
      ...
      if (isMatch) {
        for (let offset = 0; offset < phraseWords.length; offset += 1) {
          matches.push(startIndex + offset);
        }
      }
    }
...
export function HomePage({ locale }: HomePageProps) {
  ...
  const [isHeaderAtTop, setIsHeaderAtTop] = useState(() => {
  ...
  const visibleHeaderNavItems = headerNavigationItems.slice(
    0,
    visibleHeaderNavCount,
  );
  const hiddenHeaderNavItems = headerNavigationItems.slice(
    visibleHeaderNavCount,
  );
  const isHeaderMenuExpanded =
    isHeaderMenuOpen && hiddenHeaderNavItems.length > 0;
  ...
  useEffect(() => {
    ...
    const updateHeaderState = () => {
      ...
      const delta = currentScrollY - headerLastScrollYRef.current;
      const isAtTop = currentScrollY <= 8;

      setIsHeaderAtTop(isAtTop);
  ...
  useEffect(() => {
    ...
    const syncHeaderNavigation = () => {
      if (window.innerWidth < 1024) {
        setVisibleHeaderNavCount(0);
        return;
      }

      setVisibleHeaderNavCount(headerNavigationItems.length);
    };
    ...
    return () => {
      window.removeEventListener("resize", syncHeaderNavigation);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (!nav || visibleHeaderNavCount === 0) {
      return;
    }

    const frameId = window.requestAnimationFrame(() => {
      if (nav.scrollWidth > nav.clientWidth + 1) {
        setVisibleHeaderNavCount((currentCount) =>
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", closeHeaderMenuOnEscape);

    return () => {
      document.removeEventListener("mousedown", closeHeaderMenu);
      window.removeEventListener("keydown", closeHeaderMenuOnEscape);
    };
  ...
  useEffect(() => {
    ...
    const updateWords = () => {
      ...
      const rawProgress = Math.min(
        Math.max((start - paragraphCenter) / Math.max(start - end, 1), 0),
        1,
      );
      const progress = 1 - Math.pow(1 - rawProgress, 2.35);
      const revealSpread = 0.72;
      const revealWindow = 0.24;

      wordNodes.forEach((word, index) => {
        const normalizedIndex =
          wordNodes.length <= 1 ? 0 : index / (wordNodes.length - 1);
        const startAt = normalizedIndex * revealSpread;
        const endAt = Math.min(startAt + revealWindow, 1);
        const baseOpacity = word.dataset.highlighted === "true" ? 0.24 : 0.16;
        const revealProgress = Math.min(
          Math.max((progress - startAt) / Math.max(endAt - startAt, 0.001), 0),
          1,
        );
        const opacity = baseOpacity + (1 - baseOpacity) * revealProgress;
  ...
  return (
    ...
    >
      ...
      <header
        className={cn(
          "fixed inset-x-0 top-0 z-[120] transform-gpu will-change-transform transition-[transform,opacity,background-color,border-color,box-shadow] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]",
          isHeaderVisible
            ? "translate-y-0 opacity-100"
            : "pointer-events-none -translate-y-[112%] opacity-0",
          isHeaderAtTop
            ? "border-b border-transparent bg-transparent shadow-none"
            : "border-b border-black/8 bg-white shadow-[0_10px_30px_rgba(17,17,17,0.06)]",
        )}
      >
        <div
          className={cn(
            "flex h-16 w-full items-center justify-between gap-3 px-4 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] sm:h-[72px] sm:px-6 lg:gap-5 lg:px-10",
            isHeaderVisible
        ...
        >
          <div className="hidden min-w-0 flex-1 lg:flex">
            <nav
              ref={headerNavRef}
              aria-label="Primary"
              className="flex min-w-0 items-center justify-start gap-1 overflow-hidden text-[15px] font-[500] text-black"
            >
              {visibleHeaderNavItems.map((item) => (
                <a
                  key={item.href}
                  className="hero-ghost-pill focus-ring inline-flex min-h-10 shrink-0 items-center justify-center whitespace-nowrap px-4 py-2 text-[15px] font-[500] text-[#27272a]"
                  href={item.href}
                >
                  {item.label}
                </a>
          ...
          <Link
            ...
            className="shrink-0 rounded-md lg:absolute lg:left-1/2 lg:top-1/2 lg:-translate-x-1/2 lg:-translate-y-1/2"
            href="/"
            locale={locale}
          >
            <Image
              alt="Hoa Linh Studio logo"
              className="h-8 max-w-[104px] w-auto object-contain sm:h-10 sm:max-w-[132px] lg:h-11 lg:max-w-none"
              height={44}
              priority
          ...
          <div
            ref={headerMenuRef}
            className="ml-auto flex shrink-0 items-center justify-end gap-2 lg:flex-1"
          >
            <a
              className="header-cta-pill fill-black-pill focus-ring inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-full bg-[#18181b] px-4 py-2.5 text-[13px] font-[500] whitespace-nowrap text-white sm:min-h-12 sm:px-6 sm:py-3 sm:text-[15px] lg:gap-3 lg:px-7 lg:text-[16px]"
              href="#contact"
            >
            ...
            {hiddenHeaderNavItems.length > 0 && (
              <div className="relative">
                <button
                  aria-controls="header-overflow-menu"
                  aria-expanded={isHeaderMenuExpanded}
                  aria-label="Mở menu điều hướng"
                  className="fill-black-pill focus-ring inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#18181b] text-white sm:h-12 sm:w-12"
                  onClick={() => {
                ...
                {isHeaderMenuExpanded && (
                  <div
                    className="absolute right-0 top-[calc(100%+12px)] w-[220px] overflow-hidden rounded-[18px] border border-black/10 bg-white p-2 shadow-[0_20px_40px_rgba(17,17,17,0.10)]"
                    id="header-overflow-menu"
                  >
                    <div className="flex flex-col gap-1">
                      {hiddenHeaderNavItems.map((item) => (
                        <a
                          key={item.href}
                          className="focus-ring inline-flex items-center justify-between rounded-[14px] px-4 py-3 text-[15px] font-[500] text-black transition-colors duration-200 hover:bg-black/[0.04]"
                          href={item.href}
                          onClick={() => {
                            setIsHeaderMenuOpen(false);
                          }}
      ...
      <section className="hero-screen hero-screen-mobile relative isolate flex overflow-hidden bg-[#f8f7f3] pt-16 text-[#171717] sm:pt-[72px]">
        <video
          aria-hidden="true"
          autoPlay
          ...
          muted
          playsInline
          src="/videos/hero-section-background.mp4"
        />
        <div
          aria-hidden="true"
        ...
        <div className="relative z-30 flex min-h-full w-full flex-col px-4 pb-4 sm:px-5 sm:pb-5">
          <div className="flex flex-1 items-start justify-center pb-10 pt-10 sm:pb-14 sm:pt-12 lg:pt-10">
            <div className="relative z-20 mx-auto flex w-full max-w-[920px] flex-col items-center px-4 pt-14 text-center sm:pt-18 lg:pt-10">
              ...
              <Image
                alt="Hoa Linh studio"
                className="mt-2 h-auto w-[clamp(141px,52.48vw,205px)] sm:w-[clamp(205px,43.52vw,282px)] md:w-[clamp(269px,37.12vw,358px)] lg:w-[clamp(333px,30.72vw,422px)] xl:w-[clamp(397px,26.88vw,486px)]"
                height={224}
                priority
                src="/logo-hoa-linh-full.svg"
                width={760}
      ...
      >
        ...
        <div className="relative z-10 mx-auto w-full max-w-[1200px] lg:px-10 lg:py-25">
          ...
          >
            {aboutRevealWords.map((word, index) => (
              <span
                className={cn(
                  "scroll-reveal-word",
                  aboutHighlightWordIndexes.has(index) &&
                    "scroll-reveal-word--primary",
                )}
                data-highlighted={
                  aboutHighlightWordIndexes.has(index) ? "true" : undefined
                }
                key={`${word}-${index}`}
                ref={(node) => {
      ...
      >
        ...
        <div className="relative z-10 flex flex-col gap-4  sm:gap-5 sm:pb-20 lg:min-h-screen lg:justify-center lg:gap-6 lg:pb-24">
          ...
          >
            <div className="relative h-[min(calc((100vw-2rem)*9/16),360px)] w-full sm:h-[360px] lg:left-1/2 lg:h-[min(36vw,630px)] lg:w-screen lg:-translate-x-1/2">
              {focusServices.map((service, index) => {
                ...
                return (
                  <button
                    className={cn(
                      "service-carousel-card group absolute left-1/2 top-1/2 block rounded-[8px] text-left transition-[transform,width,opacity,box-shadow,filter] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] focus:outline-none sm:rounded-[10px] lg:rounded-[16px] min-[1920px]:rounded-[24px]",
                      isActive
                  ...
                  >
                    <div className="absolute inset-0 overflow-hidden rounded-[inherit]">
                      ...
                      <div
                        className={cn(
                          "absolute inset-0 rounded-[inherit] transition-[background-color,opacity] duration-500 ease-out",
                          isActive
                    ...
                    <div
                      className={cn(
                        "service-card-body absolute inset-x-0 bottom-0 z-10 flex w-full max-w-full flex-col items-center overflow-hidden px-4 pb-4 text-center text-white transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] sm:px-3 sm:pb-3 lg:px-4 lg:pb-4 min-[1920px]:px-6 min-[1920px]:pb-6",
                      )}
                    >
                      <span
                        className={cn(
                      ...
                      <h3
                        className={cn(
                          ...
                          isActive
                            ? "max-w-[82%] text-[16px] leading-[1.08] sm:max-w-[76%] sm:text-[18px] lg:max-w-[680px] lg:text-[24px] min-[1920px]:max-w-[760px]"
                      ...
                      {isActive ? (
                        <p className="service-carousel-description mt-3 w-full max-w-[700px] text-balance text-[14px] text-white/72 sm:text-[15px] min-[1920px]:max-w-[780px]">
                          {service.description}
                        </p>
                      ) : null}
                    </div>
              ...
              >
                <span
                  className={cn(
                    "service-nav-indicator pointer-events-none absolute top-1/2 right-[40px] inline-flex h-14 w-14 -translate-y-1/2 items-center justify-center rounded-full bg-[#111111] text-white shadow-[0_18px_42px_rgba(0,0,0,0.22)]",
              ...
              >
                <span
                  className={cn(
                    "service-nav-indicator pointer-events-none absolute top-1/2 left-[40px] inline-flex h-14 w-14 -translate-y-1/2 items-center justify-center rounded-full bg-[#111111] text-white shadow-[0_18px_42px_rgba(0,0,0,0.22)]",
...
function IntroLoader({ state }: { state: "visible" | "closing" }) {
  return (
    <div
      className={cn(
        "intro-loader fixed inset-0 z-[200] flex items-center justify-center bg-[#111111]",
